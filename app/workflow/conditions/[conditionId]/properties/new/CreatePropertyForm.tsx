...
export default function CreatePropertyForm({ conditionId, edgeId, sourceNodeId, existingProperties }: CreatePropertyFormProps) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            // Link property to condition
            const linkRes = await fetch(`/api/condition-properties?condition_id=${conditionId}&edge_id=${edgeId}&source_node_id=${sourceNodeId}`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ condition_id: conditionId, property_id: propertyId }),
            });

            if (linkRes.ok) {
                router.push(`/workflow/conditions/${conditionId}`);
                router.refresh();
            } else {
                alert("Erro ao vincular property à condition");
            }
        } catch (error) {
