...
import { type NodeDto, type PropertyDto } from "@/app/workflow/WorkflowTypes";
import CreatePropertyForm from "./CreatePropertyForm";

type Params = Promise<{ nodeId: string }>;

export default async function NewNodePropertyPage({ params }: { params: Params }) {
    ...
    const allProperties = Array.isArray(propertiesPayload.data) ? propertiesPayload.data : [];
            ...
            <div className="mx-auto w-full max-w-4xl">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-gray-300">
                        ...
                        <span>→</span>
                        <Link href={`/workflow/nodes/${node.id}`} className="hover:text-blue-600">Node #{node.id}</Link>
                        <span>→</span>
                ...
                <div className="mt-4">
                    <h1 className="text-xl font-bold sm:text-2xl">Adicionar Property ao Node #{node.id}</h1>
                </div>

                <CreatePropertyForm nodeId={node.id} existingProperties={allProperties} />
            </div>
        </main>
    );
}
