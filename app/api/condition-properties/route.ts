...
function parseCreateBody(body: unknown, expectedConditionId: number):
    | { ok: true; condition_id: number; property_id: number; created_at?: string; updated_at?: string }
    | { ok: false; status: number; code: string } {
...
export async function GET(req: Request) {
    ...
    const url = new URL(conditionPropsBaseUrl);
    url.searchParams.set("condition_id", String(parsed.condition_id));
...
export async function POST(req: Request) {
    ...
    const parsedBody = parseCreateBody(body, parsed.condition_id);
    if (!parsedBody.ok) {
        return NextResponse.json({ error: { code: parsedBody.code } }, { status: parsedBody.status });
    }

    const propOwn = await assertPropertyBelongsToCompany(
        propertiesBaseUrl,
        companyIdResult.company_id,
    ...
    if (!propOwn.ok) {
        return NextResponse.json(
            { error: { code: propOwn.code, details: propOwn.details } },
            { status: propOwn.status }
        );
    }
    ...
    let res: Response;
    try {
        res = await fetch(conditionPropsBaseUrl, {
            method: "POST",
            headers: {
        ...
        return NextResponse.json(
            { error: { code: "CONDITION_PROPERTIES_CREATE_FAILED", details: error instanceof Error ? error.message : error } },
            { status: 502 }
        );
    }
    ...
    if (!res.ok) {
        return NextResponse.json(
            { error: { code: "CONDITION_PROPERTIES_CREATE_FAILED", details: responseBody } },
            { status: res.status }
        );
    }
